    ...
    <section className="content-section">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {servicesData?.map((service, index) => (
          <AnimatedCard
            key={service._id}
            className="bg-gradient-to-br from-secondary-bg/80 to-secondary-bg/50 backdrop-blur-md p-6 rounded-xl border border-accent-color/30 shadow-lg transition-all duration-300 hover:border-accent-color/50 hover:shadow-accent-color/20"
            style={{ animationDelay: `${index * 100}ms` }}
          >
            <div className="w-16 h-16 mb-4 bg-gradient-to-br from-accent-color/20 to-accent-color/5 rounded-xl flex items-center justify-center border border-accent-color/30 shadow-lg">
              <i className={`${service.icon} text-3xl text-accent-color`}></i>
            </div>
            <h3 className="text-2xl title-font text-accent-color mb-3 font-mono">{service.title}</h3>
            <p className="text-text-secondary leading-relaxed">{service.description}</p>
            <div className="mt-4 pt-4 border-t border-accent-color/20">
